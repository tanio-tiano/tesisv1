...
if [ "${MLPAP_BG:-0}" != "1" ]; then
  ...
  echo "Config:  runs=${RUNS}  MaxFES=${MAXFES}  instancias=${#INSTS[@]}  numba=${NUMBA_MSG}"
  echo "Dir instancias: ${INSTANCES_DIR}"
  echo "Salida:         ${OUT}/<instance>/{base,shap}/values/"
  MLPAP_BG=1 RUNS="$RUNS" MAXFES="$MAXFES" INSTANCES_DIR="$INSTANCES_DIR" SCALES="$SCALES" \
